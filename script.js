...
function openSearch() {
    ...
    setTimeout(() => { const input = document.getElementById('searchInput'); if(input) input.focus(); }, 300);
}
...
function handleSearch() {
    ...
    const filtered = window.allProductsList.filter(p => (p.name || '').toLowerCase().includes(query) || (p.description || '').toLowerCase().includes(query) || (p.category || '').toLowerCase().includes(query));
...
function updateColorUI() {
    ...
    if(preview) preview.style.backgroundColor = colorData.hex; if(hexText) hexText.innerText = colorData.hex;
    ...
    const satVal = document.getElementById('satVal'), lightVal = document.getElementById('lightVal');
    if(satVal) satVal.innerText = `${colorData.s}%`; if(lightVal) lightVal.innerText = `${colorData.l}%`;
}
...
function openColorModal() {
    ...
    const content = document.getElementById('colorModalContent');
    if(!modal) return;
    modal.classList.remove('hidden');
    modal.classList.add('flex');
    document.body.style.overflow = 'hidden';
    setTimeout(() => {
        setWheelAngle(colorData.h || 45);
...
function searchProductsByColor(hex) {
    if(!hex) return;
    const name = colorNameFromHex(hex) || '';
    const results = (window.allProductsList || []).filter(p => {
        if(!p) return false;
        const props = [p.color, (p.colors||[]).join(' '), p.tags && p.tags.join(' '), p.name, p.description, p.category].join(' ').toLowerCase();
        if(props.includes(hex.toLowerCase().replace('#',''))) return true;
        if(props.includes(hex.toLowerCase())) return true;
        if(name && props.includes(name.toLowerCase())) return true;
        return false;
    });
    openSearch();
    const container = document.getElementById('searchResults');
    if(!container) return;
    if(results.length === 0) {
        container.innerHTML = `<div class="text-center text-gray-400 text-xs mt-10 font-serif italic">No products found for ${hex}.</div>`;
        return;
    }
    let html = '';
    results.forEach(p => {
...
function applyColor() { closeColorModal(); setTimeout(() => { showToast(`Searching styles in ${colorData.hex} ✨`, 'check-circle'); searchProductsByColor(colorData.hex); }, 300); }
...
function loadSkeletons() {
    ...
    if(catCont) catCont.innerHTML = Array(4).fill(`<div class="flex flex-col items-center gap-2"><div class="w-[76px] h-[76px] rounded-full premium-skeleton border-2"></div><div class="h-2.5 w-16 premium-skeleton rounded mt-2"></div></div>`).join('');
    const prodSkel = `<div class="w-[150px] shrink-0 bg-white rounded-xl border border-gray-100 p-2"><div class="w-full aspect-square premium-skeleton rounded-lg"></div><div class="mt-3 h-2.5 premium-skeleton rounded"></div></div>`;
    const trending = document.getElementById('trending-container'); if(trending) trending.innerHTML = Array(3).fill(prodSkel).join('');
    const newArr = document.getElementById('new-arrivals-container'); if(newArr) newArr.innerHTML = Array(4).fill(`<div class="w-full bg-white rounded-xl border border-gray-100 p-2"><div class="w-full aspect-square premium-skeleton rounded-lg"></div><div class="mt-3 h-2.5 premium-skeleton rounded"></div></div>`).join('');
}
    ...
    try {
        ...
        if(docsArr && docsArr.length > 0) {
            ...
            function updateDots(index = null) {
                const idx = index !== null ? index : Math.round(carousel.scrollLeft / carousel.offsetWidth);
                dots.forEach((dot, i) => {
                    if(i === idx) { dot.classList.add('w-4', 'bg-white'); dot.classList.remove('w-1.5', 'bg-white/50'); }
            ...
            function startBannerAutoPlay() {
                ...
                bannerAutoTimer = setInterval(() => {
                    const slidesCount = docsArr.length;
                    bannerCurrentIndex = (bannerCurrentIndex + 1) % slidesCount;
                    carousel.scrollTo({ left: bannerCurrentIndex * carousel.offsetWidth, behavior: 'smooth' });
                    updateDots(bannerCurrentIndex);
                }, 4500);
            }
            startBannerAutoPlay();
            carousel.addEventListener('mouseenter', () => clearInterval(bannerAutoTimer));
...
window.fetchCategories = async function() {
    try {
        ...
        if(arr && arr.length > 0) {
            const catCont = document.getElementById('category-container'); if(!catCont) return; catCont.innerHTML = '';
            arr.forEach((data) => {
                let img = data.image || data.imageUrl || data.url || 'https://via.placeholder.com/150';
                catCont.innerHTML += `<button class="flex flex-col items-center gap-2.5 group w-[76px] shrink-0" onclick="window.location.href='categories.html'"><div class="h-[76px] w-[76px] rounded-full overflow-hidden border border-gray-100 shadow-sm"><img src="${img}" class="w-full h-full object-cover" /></div><div class="text-[11px] font-medium text-gray-700 mt-1">${data.name}</div></button>`;
            });
        }
    } catch (error) { console.error(error); }
}
...
window.fetchProducts = async function() {
    try {
        ...
        window.allProductsList = dataArray.map(data => ({ id: data.id, name: data.name, price: data.price, mrp: data.mrp, img: data.imageMain || data.image || data.imageUrl || 'https://via.placeholder.com/300', description: data.description || '', category: data.category || '', color: (data.color || '').toString(), colors: data.colors || [], tags: data.tags || [] }));
